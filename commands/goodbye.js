...
const { channelInfo } = require('../lib/messageConfig');  // এটা add করলাম যাতে forwarding effect + join button আসে
    ...
    for (const participant of participants) {
        try {
            ...
            if (customMessage) {
                ...
                finalMessage = ` *@${displayName}* we will never miss you from *${groupName}*! ${groupDesc} 👋`;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Send goodbye image with caption (custom or default message) + channelInfo for join button
                    await sock.sendMessage(id, {
                        image: imageBuffer,
                        caption: finalMessage,
                        mentions: [participantString],
                        ...channelInfo  // এটা add করলাম যাতে নিচে channel join button/preview আসে
                    });
            ...
            // Send text message (either custom message or fallback) + channelInfo
            await sock.sendMessage(id, {
                text: finalMessage,
                mentions: [participantString],
                ...channelInfo  // এটা add করলাম যাতে নিচে channel join button/preview আসে
            ...
            // Fallback to text message + channelInfo
            const participantString = typeof participant === 'string' ? participant : (participant.id || participant.toString());
            const user = participantString.split('@')[0];
            ...
            if (customMessage) {
                ...
                fallbackMessage = `Goodbye @${user} from *${groupName}*! ${groupDesc} 👋`;
            }
           
            await sock.sendMessage(id, {
                text: fallbackMessage,
                mentions: [participantString],
                ...channelInfo  // এটা add করলাম যাতে fallback-এও button আসে
            });
        }
    }
}
